...
def register():
    if current_user.is_authenticated:
        ...
        name = request.form['firstname'].strip()\
        ...
        custom_logger('register: invalid form register, redirecting'
                      ' to register page. this probably means there is a bad'
                      ' request not aborted by valid_register_form',
                      lvl=logging.DEBUG)
...
def favmovies(imdbID=None):
    ...
    if request.method == 'GET':
        if imdbID is None:
            custom_logger('favmovies: client issued GET'
                          ' request for movies list')
            ...
            custom_logger('favmovies: client issued GET request'
                          ' for single movie')
            response = favmovies_call(user=current_user,
                                      params={'imdbID': imdbID})
            return render_template('favmovie_single.jinja',
                                   address=settings.HOST_ADDRESS,
                                   frontend_port=settings.FRONTEND_PORT,
                                   movie=response)
    elif request.method == 'POST':
        response = favmovies_call(user=current_user, method='POST',
                                  params=request.form.getlist('movies'))
        return redirect(url_for('site_views.logged'))
        ...
        response = favmovies_call(user=current_user, method='DELETE',
                                  params={'imdbID': imdbID})
        return response['Response']

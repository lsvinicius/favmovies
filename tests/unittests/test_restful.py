...
class TestSetup(TestCase):
    ...
    def create_app(self):
        app = restful_app.create_restful_app('sqlite:////tmp/test.db')
...
class RestfulTest(TestSetup):
    ...
    def call_post(self, user_id, data=None):
        ...
        return self.client.post('/restful/favmovies/'+user_id,
                                data={'movies': data})
    ...
    def test_valid_post_with_valid_user(self):
        ...
        assert len(response.json['added_movies']) == 2 and m1 is not None
    ...
    def test_valid_post_with_valid_user_twice(self):
        ...
        assert len(response.json['added_movies']) == 0 and len(m1) == 1
        assert len(m2) == 1
    ...
    def call_put(self, user_id, imdb_id, data={'comment': 'nice movie'}):
        return self.client.put('/restful/favmovies/'+user_id+'/'+imdb_id,
                               data=data)

    def test_update_comment_of_valid_movie_of_valid_user(self):
        response = self.call_put('1', 'imdbid')
        movie = Movie.query.get(1)
        assert response.json == {'Response': 'Updated successfully'}
        assert movie.comment == 'nice movie'
